 ...
 */
export async function load(threshold: number, toxicityLabels: string[]): Promise<ToxicityClassifier> {
...
export class ToxicityClassifier {
    private tokenizer: use.Tokenizer | undefined;
    private model: tfconv.GraphModel | undefined;
    private labels: string[] = new Array<string>();
    ...
    async load() {
        ...
        this.labels = model.outputs.map((d: {name: string}) => d.name.split('/')[0]);
     ...
     */
    async classify(inputs: string[] | string): Promise<Array<{label: string, results: Array<{probabilities: Float32Array, match: boolean | null}>}>> {
        ...
        return (labels as tf.Tensor2D[])
            ...
            .filter((d: {headIndex: number}) => this.toxicityLabels.indexOf(this.labels[d.headIndex]) > -1)
